...
public interface AdminRepository extends JpaRepository<Admin, Long> {
    ...
    Admin findAdministratorById(Long id);

    Admin findByCredential_Email(String email);

    @Query("SELECT d.credential FROM Admin d")
    List<Credential> findAllAdministratorCredentials();

    Admin findByCredential(Credential credential);
